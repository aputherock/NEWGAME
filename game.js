...
--------------------------------------------------------- */
  ...
  (function stampCopyrightYear() {
    ...
    document.querySelectorAll(".creditYear").forEach((el) => {
      el.textContent = year;
    });
  })();
...
--------------------------------------------------------- */
  ...
  function setAttack(name) {
    ...
    currentAttack = name;
    document
      .querySelectorAll(".atk-slot")
      .forEach((s) => s.classList.toggle("active", s.dataset.atk === name));
  }
...
--------------------------------------------------------- */
  class Particle {
    ...
    update(dt) {
      ...
      this.life -=
        dt / 1000 / (this.maxLifeSeconds || this.maxLife / 1000 || 1);
      this.life -= dt * 0.0011;
    }
    get t() {
      return clamp(this.life, 0, 1);
    }
    draw(c) {
      ...
      if (this.glow) {
        ...
        c.shadowBlur = this.size * 3 * SHADOW_SCALE;
      }
      c.fillStyle = this.color;
      const sz = this.shrink ? this.size * this.t : this.size;
      if (this.type === "dot") {
        c.beginPath();
        c.arc(this.x, this.y, Math.max(0.2, sz), 0, TAU);
        c.fill();
        ...
        c.globalAlpha = a * 0.5;
        c.beginPath();
        c.arc(this.x, this.y, sz * 2, 0, TAU);
        c.fill();
        ...
        c.beginPath();
        c.arc(this.x, this.y, sz * (2 - this.t), 0, TAU);
        c.stroke();
      }
      c.restore();
    }
  ...
  function smokePuff(x, y, count = 6) {
    ...
    for (let i = 0; i < count; i++) {
      arr.push(
        new Particle(x + rand(-8, 8), y + rand(-8, 8), {
          vx: rand(-0.4, 0.4),
          vy: rand(-0.8, -0.2),
          size: rand(8, 16),
  ...
  function lightningPath(x1, y1, x2, y2, segments = 8, spread = 26) {
    ...
    for (let i = 1; i < segments; i++) {
      const t = i / segments;
      const bx =
        lerp(x1, x2, t) + rand(-spread, spread) * (1 - Math.abs(t - 0.5) * 1.4);
      const by =
        lerp(y1, y2, t) + rand(-spread, spread) * (1 - Math.abs(t - 0.5) * 1.4);
      pts.push({ x: bx, y: by });
    }
    pts.push({ x: x2, y: y2 });
    return pts;
  ...
  function drawLightningBolt(
    ...
    lightningBolts.push({
      pts: lightningPath(x1, y1, x2, y2, 6, 14),
      life: life * 0.7,
      maxLife: life * 0.7,
      color: "#fff",
      width: 1.2,
...
--------------------------------------------------------- */
  const SFX = (() => {
    ...
    function explosion(big = false) {
      noiseBurst(big ? 0.7 : 0.4, {
        filterFreq: big ? 1800 : 1200,
        filterType: "lowpass",
        gain: big ? 0.5 : 0.3,
        sweepTo: 60,
        q: 0.7,
      });
      tone(big ? 120 : 160, big ? 0.6 : 0.35, {
        type: "sine",
        gain: big ? 0.32 : 0.2,
        sweepTo: 30,
      });
    ...
    function hit() {
      tone(140, 0.15, { type: "square", gain: 0.2, sweepTo: 60 });
    }
    function ultimate() {
      noiseBurst(1.1, {
        filterFreq: 2200,
    ...
    function levelUp() {
      tone(500, 0.25, { type: "triangle", gain: 0.2, sweepTo: 900 });
    }
    function uiClick() {
      tone(660, 0.08, { type: "triangle", gain: 0.15, sweepTo: 880 });
    }
    ...
    function resumeMusic() {
      if (bgMusicEl && !muted) bgMusicEl.play().catch(() => {});
    }
    function toggleMute() {
      muted = !muted;
...
--------------------------------------------------------- */
  class Player {
    ...
    draw(c) {
      const sx = this.x - camera.worldX,
        sy = this.y - camera.worldY;
      ...
      grd.addColorStop(0, "rgba(0,240,255,0.35)");
      grd.addColorStop(1, "rgba(0,240,255,0)");
      c.fillStyle = grd;
      c.beginPath();
      c.ellipse(0, 8, 34, 14, 0, 0, TAU);
      c.fill();
      ...
      // shoulder blades
      c.strokeStyle = "rgba(0,240,255,0.8)";
      c.lineWidth = 1.5;
      c.beginPath();
      c.moveTo(-this.r * 0.7, this.r * 0.5);
      c.lineTo(-this.r * 1.1, this.r * 0.1);
      c.stroke();
      c.beginPath();
      c.moveTo(this.r * 0.7, this.r * 0.5);
      c.lineTo(this.r * 1.1, this.r * 0.1);
...
--------------------------------------------------------- */
  ...
  class Enemy {
    ...
    get name() {
      return this.tier >= 2 ? "Enforcer Drone" : "Grid Wraith";
    }
    update(dt, player) {
      if (this.spawnT > 0) {
        this.spawnT -= dt;
      ...
      if (Math.random() < 0.05) {
        spawnParticles([
          new Particle(this.x + rand(-8, 8), this.y + rand(-8, 8), {
            vx: rand(-0.2, 0.2),
            vy: rand(-0.6, -0.1),
            size: rand(1, 2.5),
            color: this.color,
    ...
    draw(c) {
      ...
      c.fillStyle = grd;
      c.beginPath();
      c.ellipse(0, 6, this.r * 1.6, this.r * 0.7, 0, 0, TAU);
      c.fill();
  ...
  class Boss extends Enemy {
    ...
    update(dt, player) {
      ...
      if (Math.random() < 0.1) {
        spawnParticles([
          new Particle(this.x + rand(-30, 30), this.y + rand(-30, 30), {
            vx: rand(-0.3, 0.3),
            vy: rand(-1, -0.3),
            size: rand(2, 4),
            color: this.color,
    ...
    dashSlam(player) {
      const ang = angleTo(
        this.x,
        this.y,
        this.telegraphTarget.x,
        this.telegraphTarget.y,
      );
    ...
    draw(c) {
      ...
      if (this.chargeTelegraph > 0 && this.telegraphTarget) {
        const tx = this.telegraphTarget.x - camera.worldX,
          ty = this.telegraphTarget.y - camera.worldY;
        c.save();
        c.strokeStyle = "#ff2e4d";
        c.lineWidth = 3;
        c.setLineDash([8, 6]);
        c.globalAlpha = 0.6 + Math.sin(frameCount * 0.3) * 0.3;
        c.shadowColor = "#ff2e4d";
        c.shadowBlur = 12 * SHADOW_SCALE;
        c.beginPath();
        c.moveTo(sx, sy);
        c.lineTo(tx, ty);
        c.stroke();
        c.beginPath();
        c.arc(tx, ty, 50, 0, TAU);
        c.stroke();
        c.restore();
      }
      ...
      c.fillStyle = grd;
      c.beginPath();
      c.ellipse(0, 10, this.r * 1.8, this.r * 0.8, 0, 0, TAU);
      c.fill();
      ...
      c.lineWidth = 1.5;
      c.beginPath();
      c.arc(0, 0, 14 + Math.sin(frameCount * 0.05) * 3, 0, TAU);
      c.stroke();
...
--------------------------------------------------------- */
  ...
  function fireBlast(angle) {
    const speed = 11;
    const px = player.x + Math.cos(angle) * 24,
      py = player.y + Math.sin(angle) * 24;
    projectiles.push({
      x: px,
      y: py,
      ...
      r: 8,
      color: "#00f0ff",
      damage: ATTACKS.blast.damage,
      type: "blast",
      life: 1400,
      pierce: 0,
    });
    burst(px, py, "#00f0ff", 6, { minSpd: 0.5, maxSpd: 2 });
    shakeCamera(2, 60);
    SFX.blast();
  }
  ...
  function fireLightning(angle) {
    ...
    const ex = player.x + Math.cos(angle) * range,
      ey = player.y + Math.sin(angle) * range;
    ...
    const targets = enemies
      .filter(
        (e) =>
          !e.dead &&
          e.spawnT <= 0 &&
          pointNearSegment(e.x, e.y, player.x, player.y, ex, ey, 60),
      )
      .sort(
        (a, b) =>
  ...
  function fireFireball(angle) {
    ...
    projectiles.push({
      ...
      r: 14,
      color: "#ff8a1f",
      damage: ATTACKS.fireball.damage,
      type: "fireball",
      life: 1800,
      pierce: 1,
    });
  ...
  function triggerUltimate() {
    ...
    burst(player.x, player.y, "#ff2e88", 60, {
      minSpd: 3,
      maxSpd: 14,
      minSize: 2,
      maxSize: 6,
    ...
    for (let i = 0; i < 24; i++) {
      ...
      drawLightningBolt(
        player.x,
        player.y,
        player.x + Math.cos(a) * 260,
        player.y + Math.sin(a) * 260,
        "#ff8ac2",
        3,
        500,
      );
...
--------------------------------------------------------- */
  function updateProjectiles(dt) {
    ...
    projectiles = projectiles.filter(
      (p) => p.life > 0 && dist(p.x, p.y, player.x, player.y) < 1400,
    );
  }

  function drawProjectiles(c) {
    for (const p of projectiles) {
      const sx = p.x - camera.worldX,
        sy = p.y - camera.worldY;
      c.save();
      c.shadowColor = p.color;
      c.shadowBlur = 22 * SHADOW_SCALE;
      c.fillStyle = p.color;
      c.beginPath();
      c.arc(sx, sy, p.r, 0, TAU);
      c.fill();
      c.globalAlpha = 0.5;
      c.beginPath();
      c.arc(sx, sy, p.r * 1.8, 0, TAU);
      c.fill();
      c.restore();
    }
  }
...
--------------------------------------------------------- */
  ...
  function onEnemyKilled(e) {
    ...
    killfeedMsg(
      e.isBoss
        ? "★ SENTINEL PRIME DESTROYED ★"
        : `+${Math.round(base * comboMul)} · ${e.name}`,
    );
  ...
  function onPlayerDeath() {
    explosion(player.x, player.y, "#00f0ff", true);
...
--------------------------------------------------------- */
  ...
  function drawSkyline(c, horizon) {
    for (const el of bgSkyline) {
      ...
      if (el.type === "house") {
        ...
        c.lineWidth = 1.2;
        c.shadowColor = `hsla(${el.hue},100%,60%,0.8)`;
        c.shadowBlur = 6 * SHADOW_SCALE;
        c.beginPath();
        c.moveTo(hx, hy);
        c.lineTo(hx + el.w, hy);
        ...
        for (let r = 0; r < rows; r++) {
          for (let wI = 0; wI < el.windows; wI++) {
            const lit =
              Math.sin(el.winSeed + r * 3.1 + wI * 1.7 + frameCount * 0.004) >
              -0.2;
            if (!lit) continue;
            ...
            c.shadowColor = `hsla(${el.hue},100%,65%,0.9)`;
            c.shadowBlur = 5 * SHADOW_SCALE;
        ...
        c.beginPath();
        c.moveTo(sx, baseY);
        c.lineTo(baseX, baseY - el.h * 0.4);
        c.stroke();
        c.fillStyle = `hsla(${el.hue}, 55%, 12%, 0.95)`;
        c.strokeStyle = `hsla(${el.hue}, 90%, 55%, 0.35)`;
        c.lineWidth = 1;
        c.shadowColor = `hsla(${el.hue},100%,60%,0.5)`;
        c.shadowBlur = 5 * SHADOW_SCALE;
        const cx = baseX,
          cy = baseY - el.h * 0.72;
        for (const [ox, oy, r] of [
          [0, 0, el.w * 0.55],
          [-el.w * 0.4, el.w * 0.12, el.w * 0.4],
          [el.w * 0.4, el.w * 0.1, el.w * 0.4],
        ]) {
          c.beginPath();
          c.arc(cx + ox, cy + oy, r, 0, TAU);
          c.fill();
          c.stroke();
        }
        c.shadowBlur = 0;
  ...
  function drawBirds(c) {
    for (const b of bgBirds) {
      ...
      const sy =
        H * 0.32 +
        b.yOff +
        Math.sin(frameCount * 0.02 + b.bobPhase) * 6 -
        camera.worldY * b.depth * 0.2;
      ...
      c.lineCap = "round";
      c.shadowColor = "rgba(150,220,255,0.4)";
      c.shadowBlur = 3 * SHADOW_SCALE;
      c.beginPath();
  ...
  function drawBackground(c) {
    ...
    for (const s of bgStars) {
      const sx = s.x - camera.worldX * 0.3,
        sy = s.y - camera.worldY * 0.3;
      ...
      c.fillStyle = "#bde8ff";
      c.beginPath();
      c.arc(screenX, screenY, s.size, 0, TAU);
      c.fill();
    }
    c.globalAlpha = 1;
    ...
    for (const p of bgPortals) {
      const sx = p.x - camera.worldX * 0.5,
        sy = p.y - camera.worldY * 0.5;
      ...
      c.fillStyle = pgrd;
      c.beginPath();
      c.arc(sx, sy, p.r * pulse, 0, TAU);
      c.fill();
      c.strokeStyle = `hsla(${p.hue},100%,75%,0.5)`;
      c.lineWidth = 2;
      c.beginPath();
      c.arc(sx, sy, p.r * 0.55 * pulse, 0, TAU);
      c.stroke();
    }
...
--------------------------------------------------------- */
  ...
  function updateScoreUI() {
    el("scoreValue").textContent = score.toLocaleString();
  }
  ...
  function updatePowerUI() {
    ...
    el("powerText").textContent =
      ultimatePower >= 100
        ? "ULTIMATE READY"
        : `ULTIMATE ${Math.floor(ultimatePower)}%`;
    el("powerFill").style.filter =
      ultimatePower >= 100 ? "brightness(1.4) saturate(1.3)" : "none";
    document
      .querySelector(".atk-slot.ultimate")
      .classList.toggle(
        "active",
        ultimatePower >= 100 && currentAttack === "ultimate",
      );
  }
  function updateLevelUI() {
    el("levelValue").textContent = wave;
  }
  ...
  function showBossBanner(show) {
    el("bossBanner").classList.toggle("hidden", !show);
  }
...
--------------------------------------------------------- */
  ...
  function update(rawDt) {
    ...
    player.update(dt);
    camera.worldX = player.x - W / 2;
    camera.worldY = player.y - H / 2;
    updateCamera(dt);
    ...
    for (const key in cooldowns)
      if (cooldowns[key] > 0) cooldowns[key] -= rawDt;
    ...
    if (
      (mouse.down || keys["Space"] || mobileAttackHeld) &&
      currentAttack === "blast"
    )
      tryAttack();
  }
  ...
  function draw() {
    ...
    for (const b of lightningBolts) {
      ...
      ctx.shadowBlur = 16 * SHADOW_SCALE;
      ctx.lineWidth = b.width;
      ctx.beginPath();
      b.pts.forEach((pt, i) => {
        const sx = pt.x - camera.worldX,
          sy = pt.y - camera.worldY;
